...
def generate_launch_description():
    ...
    random_generator_script = os.path.join(pkg_project_gazebo, 'scripts', 'random_generator.py')

    # On exécute le script
    subprocess.run([random_generator_script], check=True)

    # 2) Maintenant on a "random_world.sdf" tout prêt
    #    On indique à gz_sim de lancer ce nouveau fichier
    random_world_path = os.path.join(pkg_project_gazebo, 'worlds', 'random_world.sdf')
